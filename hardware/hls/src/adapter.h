...
// 针对不能整除的情况，也能够正确处理的Adapter
// 其核心在于，找到最小公倍数，以其作为中间并行度进行过渡
...
template<
    class if_t,     // input type

    int T,          // input number of tokens
    int TP,         // input token parallel degree
...
public:
    ...
    static void unpk(
        ...
        hls::stream<hls::vector<if_t, TP*COP> > &o_stream
    ) {
        hls::vector<if_t, TP*CIP> vec_i(0);
        ...
        TT_LOOP: for(int tt=0; tt<TT; ++tt){
            CIT_LOOP: for(int cit=0; cit<CIT; ++cit){
                UNPK_LOOP: for(int t=0; t<UNPK_TRIP; ++t){
                    ...
                    hls::vector<if_t, TP*COP> vec_o(0);
        ...
        TT_LOOP: for(int tt=0; tt<TT; ++tt){
            COT_LOOP: for(int cot=0; cot<COT; ++cot){
                ...
                PACK_LOOP: for(int t=0; t<PACK_TRIP; ++t){
                    ...
                    hls::vector<if_t, TP*CIP> vec_i = i_stream.read();
    ...
    void non_divisible(
        ...
        hls::stream<hls::vector<if_t, TP*CP> >  packed_stream("packed_stream");

        Adapter<if_t, T, TP, C, CIP, CP>::pack  (i_stream,      packed_stream               );
        Adapter<if_t, T, TP, C, CP, COP>::unpk  (               packed_stream,      o_stream);
    }
